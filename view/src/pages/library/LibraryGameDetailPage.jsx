...
const LibraryGameDetailPage = () => {
  ...
  useEffect(() => {
    ...
    console.log("Đang tìm kiếm game với id:", numericId);
    if (isNaN(numericId)) {
    ...
    const foundGameInLibrary = storedGames.find((item) => item.id === numericId);
    if (!foundGameInLibrary) {
      alert("Game không có trong Thư viện. Đang chuyển hướng tới Thư viện.");
      navigate("/library");
      return;
    }
    ...
    if (foundGame) {
      ...
      alert("Game không tồn tại. Đang chuyển hướng tới Thư viện.");
      navigate("/library");
    }
  }, [id, navigate]);
        ...
        {game ? (
          <GameDetailContainer>
            <GameHeader>
              ...
              <Buttons>
                <RemoveButton onClick={handleRemoveFromLibrary}>Remove from Library</RemoveButton>
                <BackButton to="/library">Back to Library</BackButton>
              </Buttons>
            </GameHeader>
            ...
            {/* Bạn có thể thêm các phần tương tự như ở GameDetailPage.jsx nếu cần */}
          </GameDetailContainer>
        ) : (
